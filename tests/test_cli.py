...
from dogbass.cli import main, sync_markdown_file
...
class DogbassTests(unittest.TestCase):
    ...
    def test_sync_markdown_file_creates_post_and_writes_id(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            exit_code = sync_markdown_file(path, client)
    ...
    def test_sync_markdown_file_updates_existing_post(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_main_requires_docbase_environment_variables(self) -> None:
        ...
        with self.assertRaises(DocBaseConfigurationError):
            main(["update", "sample.md"])

    def test_main_supports_update_command(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
